...
import com.stremio.core.types.profile.UID

sealed class Event {
    data class ProfilePushedToStorage(val uid: UID) : Event()
    data class LibraryItemsPushedToStorage(val ids: List<String>) : Event()
    data class UserPulledFromAPI(val uid: UID) : Event()
    data class UserPushedToAPI(val uid: UID) : Event()
    ...
    data class UserLoggedOut(val uid: UID) : Event()
    ...
    data class SettingsUpdated(val settings: Settings) : Event()
